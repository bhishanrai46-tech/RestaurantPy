...
Variables modelled (18 input dimensions):
  Ingredient Cost, Selling Price, Sold/Week, Prep Time, Cook Time,
  Wastage %, Labour $/hr, Price Elasticity, Current Stock, Reorder Point,
  Lead Time (days), Ordering Cost, Holding Cost %/yr, Customer Rating,
  Bundle Attach %, Avg Table Time — plus computed: True Cost, Profit/Min,
  EOQ, Safety Stock, ROP, CLV, Kitchen Utilisation

Run:
    pip install streamlit pandas numpy plotly scipy openpyxl
    streamlit run menuprofit_pro.py
"""
...
[data-testid="stNumberInput"] input,[data-testid="stTextInput"] input{font-size:.88rem}
...
# ─── Default data ─────────────────────────────────────────────────────────────
# 18 input variables per dish — matches all variables from Taha + snippet
DEFAULT = {
    ...
    # ── Inventory (EOQ model — Taha Ch. 13)
...
# ── 1. TRUE COST  (ingredient + wastage + labour) ─────────────────────────────
def true_cost(row):
    """
    Full cost includes:
      real_food = ingredient × (1 + wastage%)   — real cost after spoilage
      labour    = (prep + cook) / 60 × labour_rate
    This gives a realistic food cost that includes hidden waste and kitchen time.
    """
...
# ── 2. DEMAND MODEL — price elasticity (Taha Ch. 10) ────────────────────────
def demand(p_new, p_base, qty_base, elast):
    """Power-law demand: qty = qty_base × (p_new/p_base)^elasticity"""
...
# ── 3. LP WITH SHADOW PRICES (Taha Ch. 2–3) ─────────────────────────────────
def run_lp(df, kitchen_mins, ing_budget):
    """
    Maximise contribution margin:
      max Σ (price_i − true_cost_i) × qty_i
    Subject to:
      Σ (prep_i + cook_i) × qty_i  ≤  kitchen_mins   [capacity constraint]
      Σ ingredient_i × qty_i        ≤  ing_budget      [budget constraint]
      Σ table_time_i × qty_i        ≤  kitchen_mins×2  [seating constraint]
      0 ≤ qty_i ≤ sold_i

    Dual variables (shadow prices) reveal the marginal value of relaxing
    each constraint — a direct LP insight from Taha Ch. 3.
    """
    tc       = df.apply(true_cost, axis=1).values
    cm       = (df["Price ($)"] - tc).clip(lower=0)
...
# ── 4. NONLINEAR PRICE OPTIMISER (Taha Ch. 10 — differential evolution) ──────
def optimise_prices(rows, fixed, min_mg=0.28, max_up=0.35):
    """
    Global optimisation using differential evolution (Taha §10.4).
    Objective: maximise total weekly net profit using the demand model.
    Lower bound: price ≥ true_cost × (1 + min_margin)
    Upper bound: price ≤ current_price × (1 + max_increase)
    """
...
# ── 5. EOQ INVENTORY MODEL (Taha Ch. 13) ─────────────────────────────────────
def eoq_model(row, demand_sigma_pct=0.15, service_z=1.645):
    """
    Economic Order Quantity:  EOQ = √(2DS/H)
      D = annual demand (units)
      S = cost per order ($)
      H = holding cost per unit per year = ingredient_cost × hold_pct

    Safety Stock (probabilistic demand — Taha Ch. 13):
      SS = z × σ_weekly × √(lead_time_weeks)
      z  = 1.645 for 95% service level

    Reorder Point:
      ROP = lead_time_demand + safety_stock
    """
    D  = row["Sold/Wk"] * 52                          # annual demand
    S  = row["Order Cost ($)"]                         # ordering cost
    H  = row["Ingredient ($)"] * row["Hold Cost %/yr"] / 100  # annual holding cost/unit
    if H <= 0: H = 0.01

    eoq_qty    = math.sqrt(2 * D * S / H)
    orders_pa  = D / eoq_qty                           # orders per year
    cycle_days = 365 / orders_pa                       # days between orders
    ...
    # Annual inventory cost comparison (current vs EOQ)
...
# ── 6. MENU ENGINEERING — Decision Analysis (Taha Ch. 15) ────────────────────
...
# ── 7. MONTE CARLO SIMULATION (Taha Ch. 19) ──────────────────────────────────
def monte_carlo(df, fixed=0, n=8000, sigma=0.15):
    """
    Simulate weekly profit under uncertain demand.
    Each dish's orders ~ N(base, base×sigma). Run N scenarios.
    Returns percentile distribution — the 90% CI shows the realistic revenue range.
    """
    tc   = df.apply(true_cost, axis=1).values
    cm   = (df["Price ($)"] - tc).clip(lower=0)
    qtys = df["Sold/Wk"].values.astype(float)
...
# ── 8. KITCHEN UTILISATION — Queuing proxy (Taha Ch. 18) ─────────────────────
def kitchen_analysis(df, available_mins):
    """
    Treat kitchen as M/G/1 queue (Taha Ch. 18).
    Load  = Σ (prep + cook) × orders/week
    Util  = Load / Capacity
    When utilisation > 80%, queue-theoretic wait time grows exponentially.
    """
    total_t  = (df["Prep (min)"] + df["Cook (min)"]) * df["Sold/Wk"]
    load     = total_t.sum()
    util     = load / available_mins
    seating  = (df["Table Time (min)"] * df["Sold/Wk"]).sum()

    # Most efficient dishes (highest profit/min)
    tc_vals  = df.apply(true_cost, axis=1)
    pm       = ((df["Price ($)"] - tc_vals) / (df["Prep (min)"] + df["Cook (min)"] + 0.01)).values
...
# ── 9. CUSTOMER LTV via MARKOV CHAIN (Taha Ch. 17) ───────────────────────────
def markov_ltv(rating, avg_weekly_contribution, weekly_visits=1.0, discount_rate_annual=0.10):
    """
    Two-state Markov chain: Active ↔ Churned.
    Transition probability p(retain) estimated from customer rating.
    CLV = contribution_per_visit × p_retain / (1 − p_retain + r_weekly)
    where r_weekly = annual discount rate / 52  (Taha Ch. 17 infinite horizon).
    """
    # Empirical: each 0.1 star improvement ≈ 2% retention lift
    p_retain  = min(0.96, max(0.30, 0.46 + (rating - 1.0) * 0.125))
    r_weekly  = discount_rate_annual / 52
    expected_visits = p_retain / (1 - p_retain + r_weekly)  # geometric series
...
def calc_fin(df, eng_df, fixed):
    ...
    ing_only= (df["Ingredient ($)"] * df["Sold/Wk"]).sum() / rev * 100 if rev > 0 else 0
    return dict(
...
# ═════════════════════════════════════════════════════════════════════════════
#  RECOMMENDATION ENGINE — fully algorithmic, no AI API
# ═════════════════════════════════════════════════════════════════════════════

@dataclass
class Rec:
    cat:      str    # Pricing | Inventory | Menu | Operations | Bundle
    priority: int    # 1 Critical | 2 High | 3 Medium
    dish:     str
    title:    str
    insight:  str
    action:   str
    impact:   float  # weekly $ impact
    or_ref:   str

def generate_recommendations(df, eng_df, fin, opt_prices, sh_k, sh_b, sh_s, mc, eoq_data, kitchen, fixed) -> List[Rec]:
    rows = df.to_dict("records")
    recs: List[Rec] = []

    curr_profit = total_net_profit([r["Price ($)"] for r in rows], rows, fixed)

    # ── PRICING RECOMMENDATIONS (LP + Elasticity) ───────────────────
    for i, row in enumerate(rows):
        p_c   = row["Price ($)"]
        p_o   = round(opt_prices[i] * 4) / 4   # round to nearest $0.25
        tc    = true_cost(row)
        ...
        # Below breakeven margin
        if margin < 20:
            recs.append(Rec("Pricing", 1, row["Dish"],
                f"Price is below true cost — margin only {margin:.1f}%",
                f"After accounting for {row['Wastage %']}% wastage and {(row['Prep (min)']+row['Cook (min)']):.0f} min "
                f"of labour (${tc:.2f} true cost), your margin is {margin:.1f}% — dangerously thin. "
                f"The LP model shows this dish is consuming resources at a loss.",
                f"Raise price to at least ${tc * 1.30:.2f} to reach a 23% true margin.",
                max(0, uplift) + 20,
                "LP shadow price analysis + True cost model (Taha Ch. 2–3, 10)"))

        # Significant price uplift available
            ...
            recs.append(Rec("Pricing", pri, row["Dish"],
                ...
                f"{abs((demand(p_o, p_c, row['Sold/Wk'], row['Elasticity']) - row['Sold/Wk']))/row['Sold/Wk']*100:.1f}% "
                f"at the new price, but each unit earns ${p_o-tc:.2f} vs ${p_c-tc:.2f} — a net gain.",
        ...
        # Food cost > 38%
        if fc_p > 38:
            fix_price = tc / 0.60  # target 40% food cost → price
            recs.append(Rec("Pricing", 1, row["Dish"],
                f"Food cost {fc_p:.0f}% — renegotiate or reprice",
                f"True food cost (including {row['Wastage %']}% wastage) is {fc_p:.0f}% of revenue. "
                f"Industry target is <32%. Every order sold at this ratio shrinks your safety buffer. "
                f"Shadow price analysis shows this dish locks up kitchen time worth ${sh_k:.2f}/min.",
                f"Either raise price to ${fix_price:.2f} (target 40% FC) or reduce ingredient cost by "
                f"${tc - row['Price ($)'] * 0.38:.2f}.",
                (fix_price - p_c) * row["Sold/Wk"],
                "LP constraint analysis + True cost model (Taha Ch. 3, 13)"))

        # Excellent rating — premium pricing opportunity
        if row["Rating"] >= 4.6 and pct_chg < 2:
            premium = p_c * 1.08
            extra   = (premium - tc - (p_c - tc)) * row["Sold/Wk"]
            recs.append(Rec("Pricing", 3, row["Dish"],
                ...
                f"Test a $0.50–$1.00 price increase. At current demand, +$0.75 = +${0.75*row['Sold/Wk']:.0f}/wk.",
                0.75 * row["Sold/Wk"],
                "Markov CLV model + Elasticity pricing (Taha Ch. 10, 17)"))

    # ── INVENTORY RECOMMENDATIONS (EOQ — Taha Ch. 13) ────────────────
    for i, row in enumerate(rows):
        eq = eoq_data[i]
        ...
        if eq["stockout"]:
            recs.append(Rec("Inventory", 1, row["Dish"],
                ...
                f"you need {eq['rop']:.0f} units on hand before ordering (includes {eq['safety_stock']:.0f} "
            ...
            recs.append(Rec("Inventory", 3, row["Dish"],
                ...
                f"— a real financial drag the EOQ model quantifies.",
        ...
        if eq["annual_saving"] > 50:
            recs.append(Rec("Inventory", 2, row["Dish"],
                ...
                f"({row['Hold Cost %/yr']}%/yr), saving an estimated ${eq['annual_saving']:.0f}/year.",
                f"Set standing order for {int(eq['eoq'])} units every {eq['cycle_days']:.0f} days.",
                eq["annual_saving"] / 52,
                "Economic Order Quantity formula √(2DS/H) (Taha Ch. 13)"))

    # ── MENU ENGINEERING RECOMMENDATIONS (Taha Ch. 15) ────────────────
    for i, (_, row) in enumerate(eng_df.iterrows()):
        t = row["Type"]
        pm = row["Profit/Min ($)"]

        if t == "Dog":
            kitchen_freed = (eng_df.iloc[i]["Prep (min)"] + eng_df.iloc[i]["Cook (min)"]) * row["Sold/Wk"] if "Prep (min)" in eng_df.columns else 0
            opportunity = kitchen_freed * sh_k if kitchen_freed > 0 else 0
            recs.append(Rec("Menu", 1, row["Dish"],
                f"Dog: low profit + low demand — consider removal",
                f"The menu engineering matrix (Taha Ch. 15) places {row['Dish']} in the Dog quadrant: "
                f"below-average profit (${row['True Profit ($)']:.2f}/dish) AND below-average orders ({row['Sold/Wk']:.0f}/wk). "
                f"Kitchen time freed could redeploy to Stars earning ${sh_k:.2f}/min.",
                f"Replace with a Puzzle-converted item or bundle with a Star. Removing frees ~${opportunity:.0f}/wk in kitchen capacity.",
                opportunity,
                "Menu engineering quadrant analysis (Taha Ch. 15 Decision Analysis)"))

        elif t == "Puzzle":
            target_qty = eng_df["Sold/Wk"].mean() * 0.9
            revenue_lift = (target_qty - row["Sold/Wk"]) * row["True Profit ($)"]
            if revenue_lift > 0:
                recs.append(Rec("Menu", 2, row["Dish"],
                    f"Puzzle: great margin but underordered — promote",
                    f"{row['Dish']} earns ${row['True Profit ($)']:.2f}/dish (above average) but sells only "
                    f"{row['Sold/Wk']:.0f}/wk. If demand reaches the average ({target_qty:.0f}/wk), "
                    f"weekly profit from this dish rises by ${revenue_lift:.0f}.",
                    f"Feature on specials board, add photo, train staff to upsell. Target: +{target_qty-row['Sold/Wk']:.0f} orders/wk.",
                    revenue_lift,
                    "Menu engineering quadrant (Taha Ch. 15) + Demand analysis"))

        elif t == "Plow Horse":
            # Test price uplift on popular low-margin dish
            p_test = row["Price ($)"] * 1.07
            tc_val = row["True Cost ($)"]
            extra  = (p_test - tc_val - (row["Price ($)"] - tc_val)) * row["Sold/Wk"] * 0.92  # allow 8% demand drop
            if extra > 0:
                recs.append(Rec("Menu", 2, row["Dish"],
                    f"Plow Horse: popular but thin margin — small price lift",
                    f"{row['Dish']} sells {row['Sold/Wk']:.0f}/wk (above average) but the margin is only "
                    f"{row['True Margin %']:.1f}%. The elasticity model (ε={row['Elasticity']}) shows a 7% "
                    f"price increase drops demand by only ~{abs(row['Elasticity'])*7:.1f}% — net positive.",
                    f"Raise price by $0.50–$1.00. At ${row['Price ($)']+0.75:.2f}, est. +${extra:.0f}/wk.",
                    extra,
            ...
            recs.append(Rec("Menu", 3, row["Dish"],
                ...
                f"{row['Dish']} is your highest-performing dish — top popularity AND margin. "
                f"The Markov CLV model shows high-rated Stars retain customers worth "
                f"${markov_ltv(row['Rating'], row['True Profit ($)'])[0]:.0f} lifetime value each.",
                f"Consider a 'Premium {row['Dish']}' at ${premium_p:.2f}. If 20% of orders upgrade, +${extra*0.2:.0f}/wk.",
                extra * 0.2,
                "Star quadrant strategy + Markov CLV (Taha Ch. 15, 17)"))

    # ── OPERATIONS RECOMMENDATIONS (Queuing — Taha Ch. 18) ────────────
    util_pct = kitchen["util_pct"]

    if util_pct > 85:
        recs.append(Rec("Operations", 1, "Kitchen",
            f"Kitchen at {util_pct:.0f}% — near maximum capacity",
            f"Queuing theory (M/G/1 model, Taha Ch. 18) shows that above 80% utilisation, "
            f"wait times grow exponentially. At {util_pct:.0f}%, your kitchen queue is under stress "
            f"and throughput quality degrades — directly hurting ratings and repeat visits.",
            f"Drop the two lowest Profit/Min dishes, or add 1 kitchen staff to extend capacity "
            f"by ~40 min/shift, worth ${sh_k * 40:.0f} in additional profit potential.",
            sh_k * 40,
        ...
        recs.append(Rec("Operations", 3, "Kitchen",
            ...
            f"Your kitchen has capacity headroom. The LP analysis shows each idle kitchen minute "
            f"is worth ${sh_k:.2f} in potential profit (shadow price). Low utilisation means "
            f"fixed costs are being absorbed by fewer dishes.",
            f"Add 2–3 high-profit-per-minute items to fill kitchen capacity. "
            f"Target dishes earning >${eng_df['Profit/Min ($)'].quantile(0.75):.2f}/min.",
            sh_k * (available_kitchen_mins * 0.6 - kitchen["load_mins"]) / 60 * 0.5,
    ...
    recs.append(Rec("Operations", 2, bn,
        ...
        f"This dish consumes the most kitchen minutes per week. "
        f"A prep-time reduction of even 2 minutes per order frees "
        f"{eng_df.shape[0] * 2:.0f}+ kitchen minutes per week, "
        f"worth ${sh_k * eng_df.shape[0] * 2:.0f} at current shadow price.",
        f"Batch-prep {bn} components (sauces, proteins) during off-peak hours. "
        f"Target: cut prep time by 2–3 min/order.",
        sh_k * eng_df.shape[0] * 2,
        "Bottleneck analysis + kitchen shadow price (Taha Ch. 18, 3)"))

    # Low profit/min items
    ...
    for _, row in eng_df[eng_df["Profit/Min ($)"] < pm_thresh].iterrows():
        ...
        recs.append(Rec("Operations", 3, row["Dish"],
            ...
            f"At ${row['Profit/Min ($)']:.2f}/kitchen-minute, {row['Dish']} is in the bottom quartile for "
            f"throughput efficiency. The shadow price of kitchen time is ${sh_k:.2f}/min — "
            f"this dish isn't earning its floor space.",
            f"Simplify recipe, pre-portion, or reduce portion size to cut {row['Prep (min)']+row['Cook (min)']:.0f} "
            f"min cook time by 20%. That frees ${sh_k * (row['Prep (min)']+row['Cook (min)'])*0.2 * row['Sold/Wk']:.0f}/wk.",
    ...
    for _, row in eng_df.iterrows():
        if row["Bundle Attach %"] >= 30:
            ...
            recs.append(Rec("Bundle", 3, row["Dish"],
                ...
                f"A structured 'add-on' prompt on POS or menu could lift that to "
                f"{min(55, int(row['Bundle Attach %'])+10)}%, adding ~{int(row['Bundle Sales/Wk'] * 0.25)} orders/wk.",
    ...
    if fin["safety"] < 15:
        recs.append(Rec("Operations", 1, "All Dishes",
            ...
            f"The Monte Carlo P5 scenario (${mc['p5']:,.0f} net) shows a bad week could put you "
            f"below breakeven. Only {fin['safety']:.1f}% safety margin — well below the 25% target.",
            f"Increase prices on Plow Horses by $0.50 each, target +{int(fin['bep'] - fin['surplus'])} extra orders/wk, "
            f"and cut the lowest-performing Dog dish this week.",
            abs(fin["net"] * 0.15),
    ...
    if fin["fc_pct"] > 35:
        recs.append(Rec("Pricing", 2, "All Dishes",
            ...
            f"Across all dishes, your true food cost (including wastage + labour) is {fin['fc_pct']:.1f}%. "
            f"Each percentage point above 32% costs you ~${fin['rev'] * 0.01:,.0f}/week in lost margin.",
            f"Address top 3 offenders: raise prices or reduce portion costs. "
            f"Reaching 32% adds ${(fin['fc_pct'] - 32) * fin['rev'] / 100:,.0f}/week.",
...
def fig_bcg(eng_df, avg_p, avg_q):
    ...
    for l, ax, ay, xa, ya in [
        ...
        fig.add_annotation(x=ax, y=ay, text=l, showarrow=False,
                           font=dict(size=8, color="rgba(255,255,255,.14)"), xanchor=xa, yanchor=ya)
    fig.update_layout(**DARK, height=380,
                      title="Menu Engineering Matrix — true profit vs. popularity (Taha Ch. 15)",
...
def fig_pl_waterfall(fin):
    fig = go.Figure(go.Waterfall(
        ...
        text=[f"${abs(v):,.0f}" for v in [fin["rev"],
               ...
               -fin["fixed"], fin["net"]]],
        textposition="outside", textfont=dict(size=10, color="#8A9AC8")))
    fig.update_layout(**DARK, title="True P&L Waterfall — where every dollar goes", height=280)
    return fig
...
def fig_mc(mc):
    ...
    fig = go.Figure(go.Bar(x=mid, y=cnts,
                           marker=dict(color=mid,
                                       ...
                                       showscale=False, line=dict(width=0)), opacity=.85))
    for v, l, c in [(mc["p5"],"P5  (bad week)",RED),(mc["p50"],"P50 (median)",GOLD),(mc["p95"],"P95  (good week)",GREEN)]:
    ...
    fig.update_layout(**DARK, height=260,
                      title="Monte Carlo revenue distribution — 8,000 demand simulations (Taha Ch. 19)",
...
def fig_shadow(sh_k, sh_b, sh_s):
    ...
    fig.update_layout(**DARK, height=220,
                      title="LP Shadow Prices — marginal value of relaxing each constraint (Taha Ch. 3)",
...
def fig_price_compare(rows, opt_prices, eng_df):
    ...
    fig.update_layout(**DARK, barmode="group", height=280, xaxis_tickangle=-25,
                      title="True cost vs. current vs. suggested price comparison",
...
def fig_clv(eng_df):
    ...
    fig = go.Figure(go.Bar(
        ...
        textposition="outside", textfont=dict(size=10, color="#8A9AC8")))
    fig.update_layout(**DARK, height=250, xaxis_tickangle=-25,
                      title="Customer Lifetime Value by dish — Markov retention model (Taha Ch. 17)",
...
def generate_report(df, eng_df, fin, opt_prices, mc, sh_k, sh_b, sh_s,
    ...
    for i, row in eng_df.iterrows():
        p_opt = round(opt_prices[list(eng_df.index).index(i) if i in eng_df.index else i] * 4) / 4
...
<table><thead><tr>
  ...
  <th>$/Min</th><th>Orders/Wk</th><th>Wk Profit</th><th>FC%</th><th>Rating</th>
...
<div class="foot">MenuProfit Pro &nbsp;·&nbsp; {period} Report &nbsp;·&nbsp; {rdate} &nbsp;·&nbsp; LP·EOQ·Monte Carlo·Markov applied</div>
</div></body></html>"""
...
with st.sidebar:
    ...
    demand_sigma= st.slider("Demand std dev %",           5,  30, 15,
                             help="Uncertainty in weekly demand — used in EOQ safety stock + Monte Carlo")
    st.divider()
    ...
    if uploaded:
        try:
            ...
            req = list(DEFAULT.keys())
            miss = [c for c in req if c not in imported_df.columns]
            if miss: st.error(f"Missing: {miss}"); imported_df = None
    ...
    st.caption(
        "**Elasticity guide**\n\n"
        "−0.3 – −0.6 : Drinks, coffee — low sensitivity\n\n"
        "−0.7 – −1.0 : Typical mains\n\n"
        "−1.1 – −1.5 : Price-sensitive items (salads, snacks)"
    )
...
if run_btn:
    ...
    with st.spinner("Running LP · EOQ · Monte Carlo · Markov · Queuing analysis…"):
        ...
        recs = generate_recommendations(
            ...
            mc, eoq_data, kitchen, weekly_fixed)
...
if st.session_state.ran:
    ...
    with T1:
        ...
        for col, (lbl, val, sub, cls) in zip(c2, [
            ...
            ("Kitchen Utilisation", f"{kitchen['util_pct']:.0f}%", "of available time",   "green" if kitchen["util_pct"]<75 else ("amber" if kitchen["util_pct"]<88 else "red")),
        ...
        with c_fc:
            st.plotly_chart(fig_food_cost_bars(eng_df), use_container_width=True)
        with c_clv:
            st.plotly_chart(fig_clv(eng_df), use_container_width=True)
    ...
    with T2:
        ...
        st.markdown(
            f'<div class="ibox">'
            f'<strong>LP Analysis (Taha Ch. 2–3):</strong> Your binding constraint is <strong>{binding}</strong>. '
            f'Shadow prices — kitchen minute: <strong>${sh_k:.3f}/wk</strong> · '
            f'ingredient $1: <strong>${sh_b:.4f}/wk</strong> · '
            f'seating minute: <strong>${sh_s:.4f}/wk</strong>.<br>'
            f'Differential evolution optimiser (Taha Ch. 10) found prices that add '
            f'<strong>${wk_gain:+,.0f}/wk</strong> (${wk_gain*52:+,.0f}/yr) while respecting demand elasticity.'
        ...
        st.markdown('<div class="tip">$/Min = profit per kitchen minute — the LP efficiency metric. '
                    'Dishes with low $/Min are consuming scarce capacity at below-average return.</div>',
                    unsafe_allow_html=True)

        # EOQ section
        st.markdown('<div class="sh">Inventory — Economic Order Quantity (Taha Ch. 13)</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="ibox"><strong>EOQ Model:</strong> '
            'EOQ = √(2DS/H) where D=annual demand, S=ordering cost, H=holding cost per unit/year. '
            'Reorder Point = lead-time demand + safety stock (z×σ×√lead_time). '
            f'Safety stock uses z={svc_z:.3f} for {service_lvl}% service level.</div>',
            unsafe_allow_html=True)
        eoq_df = fig_eoq_table(eng_df, eoq_data)
    ...
    with T3:
        ...
        st.markdown(
            ...
            f'<div class="pchip crit"><div class="pnum crit">{critical_n}</div><div class="plbl">🔴 Critical Actions</div></div>'
        ...
        for r in show_recs:
            ...
            imp_cls = "rec-imp" if r.impact >= 0 else "rec-imp neg"
            st.markdown(f"""
            <div class="rec {css}">
              <div class="rec-top">
                <span class="rec-cat {ccat}">{r.cat}</span>
                <span class="rec-dish">{r.dish}</span>
                <span class="{imp_cls}">+${r.impact:,.0f}/wk</span>
              </div>
        ...
        <div class="ibox" style="margin-top:1rem">
          ...
          Every recommendation is derived directly from the OR models — no AI API.<br>
          • <strong>Pricing</strong> — LP shadow prices + price elasticity demand model (Taha Ch. 3, 10)<br>
          • <strong>Inventory</strong> — EOQ formula + safety stock at {service_lvl}% service level (Taha Ch. 13)<br>
          • <strong>Menu Engineering</strong> — Decision analysis quadrant strategy (Taha Ch. 15)<br>
          • <strong>Operations</strong> — M/G/1 kitchen utilisation + shadow price of capacity (Taha Ch. 3, 18)<br>
          • <strong>Bundle</strong> — Attach rate analysis + demand uplift estimation<br>
          All dollar impacts are computed from the demand model with current elasticity values.
        </div>""", unsafe_allow_html=True)
    ...
    with T4:
        ...
        st.markdown(
            '<div class="tip">Full HTML report: true cost breakdown · LP shadow prices · '
            'EOQ inventory schedule · Monte Carlo risk · Markov CLV · all recommendations. '
            'Open in browser → Ctrl+P → Save as PDF for client or accountant delivery.</div>',
            unsafe_allow_html=True)
    ...
    <div style="text-align:center;color:#1E2840;font-size:.72rem;
      ...
      10 OR Models · 18 Input Variables · 100% Algorithmic
    </div>""", unsafe_allow_html=True)
    ...
    <div style="text-align:center;padding:52px 16px;color:#2E3858">
      ...
      <div style="font-size:1rem;line-height:1.9">
        ...
        LP · EOQ · Monte Carlo · Markov CLV · Kitchen Queuing run in &lt;3 seconds.
      </div>
    </div>""", unsafe_allow_html=True)
