...
CSS = """
<style>
html,body,[data-testid="stAppViewContainer"],.stApp{background-color:"""+BG+""" !important;color:"""+TEXT+""" !important;}
...
h1,h2,h3,h4,h5,h6{font-family:'Georgia',serif !important;font-weight:400 !important;color:"""+TEXT+""" !important;letter-spacing:-0.02em !important;}
h2{font-size:1.5rem !important;}
[data-testid="metric-container"]{background-color:"""+CARD+""" !important;border:1px solid """+BORDER+""" !important;border-radius:6px !important;padding:1.25rem 1.5rem !important;}
[data-testid="metric-container"]>div{background-color:"""+CARD+""" !important;}
[data-testid="stMetricLabel"] p,[data-testid="stMetricLabel"] div{font-size:0.72rem !important;text-transform:uppercase !important;letter-spacing:0.07em !important;color:"""+MUTED+""" !important;font-weight:600 !important;}
[data-testid="stMetricValue"],[data-testid="stMetricValue"]>div,[data-testid="stMetricValue"] div,[data-testid="stMetricValue"] *{font-family:'Georgia',serif !important;font-size:1.85rem !important;color:"""+TEXT+""" !important;font-weight:400 !important;}
.stButton>button,.stDownloadButton>button{background-color:"""+ACCENT+""" !important;color:#FFFFFF !important;border:none !important;border-radius:4px !important;padding:0.55rem 1.4rem !important;font-size:0.875rem !important;font-weight:500 !important;}
...
.stTabs [data-baseweb="tab"]{font-size:0.875rem !important;color:"""+MUTED+""" !important;padding:0.65rem 1.3rem !important;border-radius:0 !important;background:transparent !important;border:none !important;border-bottom:2px solid transparent !important;}
...
.stSelectbox>div>div,[data-baseweb="select"]>div,[data-baseweb="select"] div{background-color:"""+CARD+""" !important;color:"""+TEXT+""" !important;border-color:"""+BORDER+""" !important;}
...
.stRadio div[role="radiogroup"] label{color:"""+TEXT+""" !important;font-size:0.875rem !important;font-weight:400 !important;text-transform:none !important;letter-spacing:0 !important;}
.stMultiSelect>label{color:"""+TEXT+""" !important;}
.stMultiSelect [data-baseweb="select"] div{background-color:"""+CARD+""" !important;color:"""+TEXT+""" !important;}
[data-baseweb="tag"]{background-color:"""+ASOFT+""" !important;color:"""+ACCENT+""" !important;}
...
def section_tag(text):
    st.markdown(f"<p style='font-size:0.71rem;text-transform:uppercase;letter-spacing:0.1em;color:{MUTED};margin-bottom:0.35rem;font-weight:600;'>{text}</p>", unsafe_allow_html=True)
...
def html_table(df, max_rows=500):
    ...
    for i,(_,row) in enumerate(df.iterrows()):
        ...
        cells = "".join(
            f"<td style='padding:0.5rem 0.85rem;border-bottom:1px solid {BORDER};font-size:0.875rem;color:{TEXT};background:{bg};'>{'' if pd.isna(v) else v}</td>"
            for v in row.values)
        rows += f"<tr>{cells}</tr>"
    st.markdown(
...
def clean_df(df):
    ...
    if "revenue" in df.columns:
        df["revenue"]=pd.to_numeric(df["revenue"].astype(str).str.replace(r"[$\xa3\u20acRM,\s]","",regex=True).str.replace(r"[^\d.\-]","",regex=True),errors="coerce")
...
def render_sidebar():
    with st.sidebar:
        st.markdown(f"<div style='padding:1rem 0 1.25rem;'><p style='font-family:Georgia,serif;font-size:1.2rem;color:{TEXT};margin:0;letter-spacing:-0.02em;'>Seralung Opti</p><p style='font-size:0.72rem;color:{MUTED};margin-top:0.1rem;'>Cafe Business Intelligence</p></div>",unsafe_allow_html=True)
        st.markdown("---")
        ...
        st.markdown("---")
        st.markdown(f"<p style='font-size:0.7rem;color:{MUTED};'>v1.0.0 · Seralung Opti</p>",unsafe_allow_html=True)
            ...
            if mdf.empty: callout("No items with cost data found.","info")
                ...
                if not hi.empty:
                    ...
                    for col,(_,r) in zip(hcols,hi.iterrows()):
                        with col: card_html(f"<p style='font-size:0.82rem;font-weight:600;color:{TEXT};margin:0 0 0.25rem;'>{r['item_name'].title()}</p><p style='font-family:Georgia,serif;font-size:1.15rem;color:{TEXT};margin:0;'>{fmt_pct(r['margin_pct'])}</p><p style='font-size:0.72rem;color:{MUTED};margin:0.15rem 0 0;'>{r['tier']} performer</p>")
    ...
    for rt in ["pricing","menu","marketing","operations"]:
        ...
        for rec in grp:
            ...
            html_rec = (
                ...
                f"<p style='font-family:Georgia,serif;font-size:0.95rem;color:#1C1C1C;margin:0;'>{title_txt}</p>"
                ...
                f"<p style='font-size:0.875rem;color:#5A5856;margin:0 0 0.4rem;line-height:1.6;'>{detail_txt}</p>"
                f"<p style='font-size:0.78rem;color:#5A5856;margin:0;font-style:italic;'>Expected impact: {impact_txt}</p></div>"
            )
